...
function softuni_jobs_location_taxonomy() {
	$labels = array(
		'name'              => _x( 'Locations', 'taxonomy general name', 'softuni' ),
...
/**
 * Register a 'company' taxonomy for post type 'job', with a rewrite to match book CPT slug.
 *
 * @see register_post_type for registering post types.
 */
function softuni_jobs_company_taxonomy() {
	$labels = array(
		'name'              => _x( 'Companies', 'taxonomy general name', 'softuni' ),
		...
		'update_item'       => __( 'Update Company', 'softuni' ),
